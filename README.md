...
当前源码为 **0.4.0 候选版，尚未公开发布**，新增探头、收回和贴边停靠，共 11 个动作状态。已公开的 0.3.1 仍可使用；市场和既有 Release 的安装包不会因候选源码更新而自动改变。
...
共 87 帧 RGBA8 透明 PNG。原有八个动作的 62 帧仍为 512 × 512，连同图标逐字节保留；新增探头、收回各 12 帧，贴边停靠 1 帧。新增状态使用相同的 288 × 288 画布，构建检查总图片字节不超过 8 MiB、解码像素不超过 32 Mi。
...
下面的演示及面板预览循环播放，便于查看；桌面中的打招呼、醒来、探头和收回按单次动作处理。探头末帧、停靠画面与收回首帧完全相同，使停靠接缝连续。送东西是抱着物品原地迈步的动作，实际移动、贴边和串门行为由宿主负责。部分动作之间仍有轻微造型差异，六帧动作的流畅度也有限。
